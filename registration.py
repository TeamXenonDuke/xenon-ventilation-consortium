...
def register_ants_python(
    ...
    Args:
        image_static: np.ndarray static image.
        image_moving1: np.ndarray moving image 1. Here, the image_moving1 is
        registered to the registered image_moving2.
        image_moving2: np.ndarray moving image registered to image_static.
    ...
    static_path = ants.from_numpy(image_static)
    moving2_path = ants.from_numpy(image_moving2)
    moving1_path = ants.from_numpy(image_moving1)

    logging.info("*** Using Python version of Ants Registration ...")
    moving2_registered = ants.registration(
        static_path,
        moving2_path,
        ...
        aff_iterations=(20, 20, 20),
        aff_shrink_factors=(4, 2, 1),
        aff_smoothing_sigmas=(0, 0, 0),
    )

    moving2_reg = moving2_registered["warpedmovout"].numpy().astype("bool")

    # Now, registering UTE and Mask (registered mask with Vent). Fixed - Mask
    # Moving - UTE
    moving1_registered = ants.registration(
        moving2_registered["warpedmovout"],
        moving1_path,
        type_of_transform="Rigid",
        outprefix="moving",
        grad_step=0.1,
    ...
    moving1_reg = moving1_registered["warpedmovout"].numpy()
    # registration currently not working
    moving1_reg = image_moving1
    return moving1_reg.astype("float64"), moving2_reg.astype("bool")
